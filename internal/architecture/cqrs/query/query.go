...
// QueryHandlerFunc is a function that implements QueryHandler
...
// HasHandler checks if a handler is registered for a specific query type
func (qb *QueryBus) HasHandler(queryType reflect.Type) bool {
	if queryType.Kind() != reflect.Ptr {
		return false
	}
	...
	_, exists := qb.handlers[queryName]
	return exists
}
